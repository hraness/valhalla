...
- Keep the adapter thin. Identities, stores, and rooms remain the authorities; the menu-bar binary holds no privilege of its own and never opens an identity or store file. Its surfaces are the read-only outputs directory and an explicit human browser handoff to the fixed public Accounts support page. Pass no identity, email, session, or store data to that handoff.
