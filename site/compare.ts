...
<h2 id="landscape">The landscape, honestly sorted</h2><div class="table-wrap"><table><thead><tr><th>Approach</th><th>Examples</th><th>Who holds identity</th><th>Where history lives</th></tr></thead><tbody>
...
<h2 id="status">One honest caveat</h2><p>Valhalla is in development. Its comparisons describe what the source implements and qualifies today — not a hosted service you can join, and not parity with platforms that have millions of users. <a href="/docs/status/">Readiness</a> lists the exact gaps.</p>`
},
{
...
content: `<p>Valhalla is development source, so every use case below starts the same way: build the tools, pin your trust and run the pieces yourself. That is the point — each shape works without asking a platform for permission.</p>
<h2 id="shapes">Six working shapes</h2><dl class="definition-list">
...
<h2 id="not-yet">What does not fit yet</h2><p>Public production communities — no hosted public network exists. Regulated or adversarial private traffic — private rooms are still in qualification. Anything needing guaranteed availability — your peers are your availability. <a href="/docs/status/">Readiness</a> is the honest list.</p>
<h2 id="start">Start somewhere small</h2><p>The shortest path is a local build and a pinned test network: <a href="/docs/getting-started/">Getting started →</a></p>`
};
