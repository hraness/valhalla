...
- `pages.ts` owns documentation pages with their Diátaxis kinds; `compare.ts` owns comparison and use-case pages; `writing.ts` owns notes and research articles; `articles.ts` owns the reviewed technique posts (bodies in `articles/<slug>.md`, review records in `article-admissions.ts`, validated by `articles.test.ts`); `discovery.ts` adds indexable posts to the sitemap, `llms.txt` and the Atom feed at `/writing/feed.xml`; `docs.ts` renders every collection with grouped navigation, breadcrumbs and per-page JSON-LD.
