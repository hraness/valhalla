...
| Native retries retain exact transport identity, uncertainty and attempt evidence | `DeliveryStore::tick`, outcome publication, `resume` and reopen | [Native delivery model](native-delivery/README.md); existing delivery tests and the real TLS lost-receipt/resume regression; `native-delivery/delivery.rs` inductive Verus proof of all seven checked invariants over `normal.cfg` and `uncertain-write.cfg`, with proved counterexamples for each mutant and a completion witness | One exact job, two attempts per allowance, one explicit resume, bounded outages/crashes and one precommit outcome refusal. Atomic local publication is assumed; postcommit barrier/readback failure, delivery liveness, cancellation and re-encryption are outside this model. The Verus proof strengthens the *model* claims to induction over the instance; production correspondence still rests on the named tests. |
