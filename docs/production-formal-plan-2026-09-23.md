...
Focused owners run the relevant crate regressions and qualification harnesses once for their exact inputs and return commands/results. Use Rust 1.98.1 on this host and the retained shared target directory `/private/tmp/valhalla-steel-20260922/.verify-build`; the previously documented `/private/tmp/valhalla-target-shared` is absent. Set the compiler path explicitly so Homebrew Rust 1.97.1 cannot be selected accidentally. Never let concurrent builds poison a shared target. Route heavyweight work through the installed absolute `/Users/benguo/.bun/bin/hra-host-run`, respecting repository scheduling and separate browser-auth/mac-native ownership. Do not hold a compute slot during external CI waits.
