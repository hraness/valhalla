...
fn contact_single_secret_file_initializes_fresh_recipient_after_pinned_inspection() {
    block_on(async {
        ...
        let secret = pair
            ...
            .await
            .unwrap();
        // v3 offers retain bounded predecessor-signed succession proofs; an
        // empty chain costs one count byte and the bound covers 16 proofs.
        assert_eq!(secret.confidential_bytes().len(), 714);
        assert!(secret.confidential_bytes().len() <= crate::contact::MAX_OFFER_BYTES);
        assert_eq!(crate::contact::MAX_OFFER_BYTES, 10970);
        assert!(matches!(
