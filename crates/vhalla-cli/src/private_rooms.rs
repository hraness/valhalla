...
/// Relay listeners are explicit numeric IP:port pairs; no DNS or ambient host.
...
/// Client-side relay endpoints accept an explicit IP:port or a DNS
/// name:port resolved at use; TLS still pins identity through --tls-ca
/// and --tls-name, never through the resolved name.
fn relay_peer(args: &Args, name: &str) -> Result<std::net::SocketAddr, String> {
    crate::endpoint::Endpoint::parse(args.text(name)?)?.resolve()
}
