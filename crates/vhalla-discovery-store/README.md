...
and optional `state.tmp`. Owner modes are 0700 directories / 0600 regular files;
...
Publication writes a complete checksummed intent, synchronizes it and its
directory, writes/synchronizes a temporary exact candidate, renames it over the
...
Focused native tests exercise nine actual filesystem publication cut points,
