...
### Shared Dioxus web and desktop application — cancelled

The Dioxus-based web and desktop UI experiments were retired. The associated prototype directories (`dioxus-ui`, `dioxus-services`, `dioxus-controller`, `dioxus-boundary`, `dioxus-native-closed`, `dioxus-preview-assembler`, and `room-directory-ui`) were removed because the client UI is not on the near-term roadmap and the dependency surface was too large to carry without active qualification. The layered ownership design (pure `vhalla-*` crates, `vhalla-client-api`, `vhalla-client`, `vhalla-ui`, `vhalla-web`, `vhalla-desktop`) remains a valid architecture pattern but is unowned. A future client effort can re-adopt this shape with a smaller, explicitly reviewed renderer boundary.

The security-first boundaries documented above (inert presentation data, no renderer dependency in the engine, typed host effects, and no execution of untrusted content) still apply to any later client framework.
