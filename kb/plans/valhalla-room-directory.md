...
| R5 | Shared Dioxus room directory/creation UI; genuine browser/native journey, offline pending and stale collision UX | `prototypes/room-directory-ui` implements the shared component tree over the `RoomServices` trait: directory/search, room detail, account, and creation screens render `vhalla-rooms-app` projections only — no registry, journal, key or consensus type crosses the boundary. The pending strip surfaces every marker resolution (queued/in flight/name taken/not admitted) and the creation form drops signed bodies through `submit` — never signs. `NodeServices` (unix) adapts the real replica `Service` with `sync` inside `project`; `FixtureServices` backs SSR tests and the fixture launchers. Six `dioxus_ssr` tests assert real rendered output for every route including not-found and the collision surface; the `vhalla-rooms-desktop` launcher compiles on macOS (wry/tao). Browser validation is CI-deferred: `dioxus-web` builds only for `wasm32` and the local toolchain lacks the wasm std. The app crate is now split portable-DTO/unix-machinery so web renderers consume projections without the filesystem surface |
