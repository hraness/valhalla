//! The Platonik corpus for spikes and vectors: conversion from the pinned
//! engine's experiment type into the restated types, and deterministic
//! generators of valid v1 experiments. Available with the `std` feature only;
//! the library core never depends on `platonik-core`.
...
/// The Platonik protocol id this corpus restates.
...
/// Converts a Platonik program; panics on a v1-invalid program.
...
/// A Platonik experiment split into the restated world, case, and assignment.
pub struct Converted {
    /// The validated world.
    pub world: World,
    /// The validated case with its declared loading work.
    pub case: Case,
    /// The validated programs.
    pub assignment: Assignment,
    /// The JSON byte length Platonik charges as loading.
    pub loading_work: u64,
}

/// Converts a v1 experiment, declaring its JSON byte length as loading work.
...
/// A splitmix64 generator for deterministic corpus generation.
pub struct Rng(u64);
