...
fn parse_args(argv: &[String]) -> Result<Args, String> {
    ...
    while i < argv.len() {
        ...
        if let Some(name) = arg.strip_prefix("--") {
            if name == "deep" || name == "help" || name == "no-context" {
