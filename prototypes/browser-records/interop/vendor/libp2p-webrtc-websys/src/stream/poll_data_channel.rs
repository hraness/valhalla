...
// Diagnostic candidate: detach callbacks before the final Rust closure owner
// disappears. Clone shares those owners, so earlier clone drops do nothing.
impl Drop for PollDataChannel {
    fn drop(&mut self) {
        if Rc::strong_count(&self._on_open_closure) == 1 {
